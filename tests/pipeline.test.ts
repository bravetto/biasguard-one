...
import { one, oneFormatted } from "@biasguard/one";
...
for (const testCase of TEST_CASES) {
  ...
  if (result.score) {
    console.log(
      `\n${result.reflections
        .slice(0, 2)
        .map((r) => `  🔍 ${r.mirror}: ${r.reflects.slice(0, 55)}...`)
        .join("\n")}`,
    );
    ...
    console.log(
      `  ║  Bias Score: ${result.score.score}/100 (${result.score.severity.toUpperCase()})`,
    );
    console.log(`  ║  Primary: ${result.score.primaryBias}`);
    console.log(
      `  ║  Fallacies: ${result.score.relatedFallacies.join(", ") || "None"}`,
    );
    console.log(
      `  ║  Domains: ${result.score.impactDomains.join(", ") || "General"}`,
    );
    console.log(
      `  ╠════════════════════════════════════════════════════════════╣`,
    );

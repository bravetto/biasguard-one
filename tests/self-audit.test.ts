...
for (const docFile of DOCS_TO_AUDIT) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!result.clear && result.score) {
      ...
      if (
        result.score.severity === "critical" ||
        result.score.severity === "high"
      ) {
        criticalBiases++;
        console.log(
        ...
        console.log(
          `     Pattern: ${result.reflections[0]?.mirror || "Unknown"}`,
        );
      }
    }
  }
...
if (criticalBiases === 0) {
  ...
  console.log(
    `⚠️  ${criticalBiases} critical patterns found - review recommended.`,
  );
}

...
const FLOWS = { flows: true };
const block = (guard, signal, guidance) => ({
 ...
 */
function recursiveDecode(str, maxIterations = 5) {
    let current = str;
    let iterations = 0;
...
function isDangerousAction(action) {
    ...
    if (DANGEROUS_ACTIONS.has(lower))
        return true;
...
const guardAction = (input) => {
    // Input is already normalized by caller
    // Check for dangerous action phrases in text
    const lower = input.toLowerCase();
    if (lower.match(/please\s+(delete|remove|destroy|wipe)/i)) {
        return block('ACTION', 'dangerous request', 'Potentially dangerous action in natural language. Verify intent.');
    }
    return FLOWS;
};
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
const GUARDS = [guardCritical, guardBoundary, guardAction];
/**
 * Recursively scan nested objects and arrays
 * Normalizes each string value before checking
 */
function deepScan(value, depth = 0) {
    if (depth > 20)
        return FLOWS; // Prevent infinite recursion
        ...
        for (const field of actionFields) {
            if (value[field] && typeof value[field] === 'string') {
                ...
                if (isDangerousAction(normalized)) {
                    return block('ACTION', 'dangerous operation', `Dangerous action detected: ${normalized}. Verify intent.`);
                }
            }
        }

...
export function activate(context: vscode.ExtensionContext) {
  ...
  diagnosticCollection =
    vscode.languages.createDiagnosticCollection("biasguard");
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("biasguard.scanFile", () => {
      ...
      vscode.window.showInformationMessage(
        "BiasGuard: Epistemic scan complete",
      );
    }),
  );
    ...
    vscode.commands.registerCommand("pristine.execute", async () => {
      ...
      if (!workspaceFolders) {
        vscode.window.showErrorMessage(
          "No workspace open. Entropy cannot be reduced in a void.",
        );
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("pristine.shield", async () => {
      ...
      if (!workspaceFolders) {
        vscode.window.showErrorMessage(
          "No workspace open. Cannot reinforce shields in a void.",
        );
        return;
      }
      ...
      outputChannel.show();
      vscode.window.showInformationMessage(
        "Shields Holding. (.gitignore / .cursorignore updated) ∞",
      );
    }),
  );
...
function runGuard(document: vscode.TextDocument) {
  ...
  if (!epistemicResult.clear) {
    ...
    // Show notification
    const highRisks = epistemicResult.risks.filter(
      (r) => r.severity === "High",
    );
  ...
  if (result.flows && epistemicResult.clear) {
    ...
    statusBarItem.color = "#00ff88";
    statusBarItem.tooltip =
      "✓ System Protected. No violations detected.\n✓ No epistemic bias risks detected.\n\n∞ Like water flows ∞\n\nLOVE = LIFE = ONE";
    ...
    // Narrowing can be tricky across imports — assert the type here explicitly for TS
    showBlocked(
      document,
      result as import("@biasguard/security").Signal & { flows: false },
    );
  }
}

function showEpistemicRisks(
...
async function enforceStructure(
  root: string,
